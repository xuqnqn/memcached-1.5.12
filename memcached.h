...
typedef struct {
    ...
    int notify_receive_fd;      /* receiving end of notify pipe */          /* 读pipe管道文件描述符 */
    int notify_send_fd;         /* sending end of notify pipe */            /* 写pipe管道文件描述符 */
    struct thread_stats stats;  /* Stats generated by this thread */        /* 线程统计信息结构体,每个线程都会有自己的统计信息 */
    struct conn_queue *new_conn_queue; /* queue of new connections to handle */     /* 当前线程待处理的连接队列,主线程负责将新的连接插入到该队列 */
...
struct conn {
    ...
    char   *rbuf;   /** buffer to read commands into */                                 /* 网络连接读取的数据存放缓冲区地址 */
    char   *rcurr;  /** but if we parsed some already, this is where we stopped */      /* 当前读取缓冲区的位置 */
    int    rsize;   /** total allocated size of rbuf */                                 /* 每次从网络连接读取多少数据到缓冲区 */
    int    rbytes;  /** how much data, starting from rcur, do we have unparsed */       /* 缓冲区剩余待处理的字节数 */
    ...
    char   *ritem;  /** when we read in an item's value, it goes here */                /* 指向缓冲区待处理的数据位置 */
    ...
    void   *item;     /* for commands set/add/replace  */       /* 指向内存item指针  */
...
#endif
    enum protocol protocol;   /* which protocol this connection speaks */               /* 协议包类型 (字符串、二进制) */
    enum network_transport transport; /* what transport is used by this connection */   /* 网络连接类型 (TCP、UDP) */
    ...
    bool   noreply;   /* True if the reply should not be sent. */                       /* 是否给客户端答复状态 */
    ...
    short cmd; /* current command being processed */        /* 当前的命令类型(get、set、add) */
    int opaque;
    int keylen;       /* key长度 */
    conn   *next;     /* Used for generating a list of conn structures */
    LIBEVENT_THREAD *thread; /* Pointer to the thread object serving this connection */     /* 当前连接属于那个线程的,保存对应线程的指针 */
};
